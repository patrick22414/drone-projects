...
int main(int argc, char** argv)
{
    ...
    const Action::Result set_alt_result = action->set_takeoff_altitude(takeoff_altitude);
    if (set_alt_result != Action::Result::SUCCESS) {
        std::cout << ERROR_CONSOLE_TEXT << "Set takeoff altitude failed:" << Action::result_str(set_alt_result)
                  << NORMAL_CONSOLE_TEXT << std::endl;
        return 1;
    } else {
        std::cout << NORMAL_CONSOLE_TEXT << "Takeoff altitude set to " << action->get_takeoff_altitude().second << "m"
                  << NORMAL_CONSOLE_TEXT << std::endl;
    }
